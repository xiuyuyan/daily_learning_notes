...
class Model(nn.Module):
    def __init__(self, configs, max_seq_len: Optional[int] = 1024, d_k: Optional[int] = None,
        ...
        if self.decomposition:
            self.decomp_module = series_decomp(kernel_size) # 分解为残差项和趋势项
    ...
    def forward(self, x):
        """
        input:[batch_size,input_length,channel]
        中间的转置会还原
        output:[batch_size,input_length,channel]

        permute函数：调整为度顺序
        """
        if self.decomposition:
            res_init,trend_init = self.decomp_module(x) # 分解为残差项和趋势项
