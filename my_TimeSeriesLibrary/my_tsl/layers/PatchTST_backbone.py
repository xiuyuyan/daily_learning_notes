...
class PatchTST_backbone(nn.Module):
    def __init__(self, c_in: int, context_window: int, target_window: int, patch_len: int, stride: int,
        ...
        if padding_patch == 'end':
            self.padding_patch_layer = nn.ReplicationPad1d(patch_num)
            patch_num += 1
        ...
        """
        head输出头：
            核心功能是将编码器提取的特征映射到目标预测空间，支持两种模式：预训练专用头和展平头
        """
        self.head_nf = d_model * patch_num
        self.n_vars = c_in
        self.pretrain_head = pretrain_head
        self.head_type = head_type
        self.individual = individual # 是否为个体头（多变量独立预测）
    ...
    def create_pretrain_head(self, head_nf, vars, dropout):
        return nn.Sequential(nn.Dropout(dropout),
                             nn.Conv1d(head_nf, vars, kernel_size=1))
